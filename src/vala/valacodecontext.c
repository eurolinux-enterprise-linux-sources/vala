                                       ...
                                       gboolean cmdline)
{
	...
	if (_tmp3_) {
		...
		_tmp8_ = vala_unresolved_symbol_new (NULL, "GLib", NULL);
		_tmp9_ = _tmp8_;
		_tmp10_ = vala_using_directive_new ((ValaSymbol*) _tmp9_, NULL);
		_tmp11_ = _tmp10_;
		_vala_code_node_unref0 (_tmp9_);
		ns_ref = _tmp11_;
		_tmp12_ = source_file;
		_tmp13_ = ns_ref;
		vala_source_file_add_using_directive (_tmp12_, _tmp13_);
		_tmp14_ = vala_code_context_get_root (self);
		_tmp15_ = _tmp14_;
		_tmp16_ = ns_ref;
		vala_namespace_add_using_directive (_tmp15_, _tmp16_);
		_tmp17_ = source_file;
		vala_code_context_add_source_file (self, _tmp17_);
		...
		if (g_str_has_suffix (filename, ".vapi")) {
			...
			_tmp18_ = g_str_has_suffix (filename, ".gir");
		}
		if (_tmp18_) {
			ValaSourceFile* source_file = NULL;
			const gchar* _tmp19_;
			...
			_tmp19_ = rpath;
			_tmp20_ = vala_source_file_new (self, VALA_SOURCE_FILE_TYPE_PACKAGE, _tmp19_, NULL, cmdline);
			source_file = _tmp20_;
			_tmp21_ = source_file;
			vala_source_file_set_relative_filename (_tmp21_, filename);
			_tmp22_ = source_file;
			vala_code_context_add_source_file (self, _tmp22_);
			_vala_source_file_unref0 (source_file);
		} else {
			if (g_str_has_suffix (filename, ".c")) {
				const gchar* _tmp23_;
				_tmp23_ = rpath;
				vala_code_context_add_c_source_file (self, _tmp23_);
			} else {
				if (g_str_has_suffix (filename, ".h")) {
				} else {
					gchar* _tmp24_;
					gchar* _tmp25_;
					_tmp24_ = g_strdup_printf ("%s is not a supported source file type. Only .vala, .vapi, .gs, and .c" \
